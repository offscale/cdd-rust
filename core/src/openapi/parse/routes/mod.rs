...
use crate::openapi::parse::models::{ParsedRoute, RouteKind};
use crate::openapi::parse::normalization::{normalize_boolean_schemas, normalize_nullable_schemas};
use crate::openapi::parse::ref_utils::normalize_ref_to_local;
use crate::openapi::parse::registry::DocumentRegistry;
use crate::openapi::parse::routes::builder::parse_path_item;
use crate::openapi::parse::routes::shims::{ShimComponents, ShimOpenApi, ShimPathItem, ShimServer};
use crate::openapi::parse::schemas::refs::compute_base_uri;
use crate::openapi::parse::validation::{validate_component_keys, validate_openapi_root};
use serde_json::json;
...
fn parse_openapi_routes_from_value(
    ...
    upgrade_swagger2_to_openapi3(json_val);
...
fn resolve_operation_ref_to_path(
    ...
    let (key, path_item) = match first {
        "paths" | "webhooks" => {
            ...
            let key = crate::openapi::parse::ref_utils::decode_pointer_segment(path_segment);
        ...
        "components" => {
            ...
            let name = crate::openapi::parse::ref_utils::decode_pointer_segment(name_seg);
            ...
            if let Some(method_seg) = method_segment {
                let method = crate::openapi::parse::ref_utils::decode_pointer_segment(method_seg);
    ...
    if let Some(method_segment) = segments.next() {
        ...
        let method = crate::openapi::parse::ref_utils::decode_pointer_segment(method_segment);
        if !path_item_supports_method(&path_item, &method) {
            return None;
        }
    }
...
fn path_item_ref_matches(ref_str: &str, self_uri: Option<&str>, target_name: &str) -> bool {
    ...
    crate::openapi::parse::ref_utils::decode_pointer_segment(name_seg) == target_name
}
...
impl ComponentRefResolver {
    ...
    fn resolve_component_ref(
        ...
        let name =
            crate::openapi::parse::ref_utils::extract_component_name(ref_str, self_uri, section)
                .ok_or_else(|| {
...
fn resolve_path_item_ref_inner(
    ...
    if let Some(pointer) = normalized.strip_prefix("#/") {
        ...
        if segments.first() == Some(&"paths") {
            ...
            let path_key = crate::openapi::parse::ref_utils::decode_pointer_segment(name_seg);
...
mod tests {
    use super::*;
    use crate::openapi::parse::models::{ParamSource, RuntimeExpression, SecuritySchemeKind};
    use crate::openapi::parse::routes::shims::ShimOpenApi;
...
"#;
        ...
        assert!(route.path_params[0].source == crate::openapi::parse::models::ParamSource::Path);
        assert!(
            route.path_servers.as_ref().map(|s| s[0].url.as_str())
                == Some("https://api.example.com/v2")
        );
        assert!(route.params.iter().any(
            |p| p.name == "id" && p.source == crate::openapi::parse::models::ParamSource::Path
        ));
        assert!(route.params.iter().any(
            |p| p.name == "q" && p.source == crate::openapi::parse::models::ParamSource::Query
        ));
    }
...
"#;
        ...
        assert_eq!(
            routes[0].kind,
            crate::openapi::parse::models::RouteKind::Webhook
        );
    }
            ...
            if let Some(path_obj) = path_item.as_object_mut() {
                for (_, operation) in path_obj.iter_mut() {
                    if let Some(op_obj) = operation.as_object_mut() {
                        // Upgrade Responses
                        if let Some(responses) = op_obj.get_mut("responses").and_then(|r| r.as_object_mut()) {
                            for (_, response) in responses.iter_mut() {
                                if let Some(resp_obj) = response.as_object_mut() {
                                    if let Some(schema) = resp_obj.remove("schema") {
                                        ...
                                        content.insert("application/json".to_string(), serde_json::Value::Object(media));
                        ...
                        if let Some(parameters) = op_obj.get_mut("parameters").and_then(|p| p.as_array_mut()) {
                            ...
                            if let Some(idx) = body_param_idx {
                                ...
                                if let Some(mut body_obj) = match body_param { serde_json::Value::Object(o) => Some(o), _ => None } {
                                    ...
                                    op_obj.insert("requestBody".to_string(), serde_json::Value::Object(request_body));
                                }
                            }
                        }
                    }
